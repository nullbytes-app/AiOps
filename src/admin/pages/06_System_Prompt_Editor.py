...
# Page configuration
st.set_page_config(
    page_title="System Prompt Editor",
    page_icon="✏️",
    layout="wide",
...
async def async_api_call(
    ...
    """
    base_url = "http://localhost:8000"  # Will be configurable via environment
    try:
        async with AsyncClient(timeout=timeout) as client:
            if method.upper() == "GET":
                ...
                response = await client.put(f"{base_url}{endpoint}", json=json_data)
        ...
        if not st.session_state.templates_list:
            with st.spinner("Loading templates..."):
                ...
                if templates_data and "data" in templates_data:
                    st.session_state.templates_list = templates_data["data"]
            ...
            if template:
                with st.expander("📖 Template Info"):
                    ...
                    if template.get("is_builtin"):
                        st.badge("Built-in", "primary")
                    else:
                        st.badge("Custom", "secondary")
        else:
