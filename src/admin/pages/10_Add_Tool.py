...
async def async_api_call(
    ...
    async with AsyncClient(base_url=base_url, timeout=timeout) as client:
        try:
            ...
            # Show more specific error message for duplicate tool names (409 Conflict)
            if e.response.status_code == 409:
                st.error(f"⚠️ Duplicate Tool: {error_detail}")
            else:
...
def load_tenants() -> list[tuple[str, str]]:
    """
    Load all active tenants from database.

    Returns:
        List of tuples (tenant_id, tenant_name) for dropdown selection
    """
    try:
        from src.admin.utils.tenant_crud_helpers import get_all_tenants

        tenants = get_all_tenants(include_inactive=False)
        return [(t.tenant_id, t.name) for t in tenants]
    except Exception as e:
...
def render_auth_config_form(security_schemes: dict[str, Any]) -> dict[str, Any]:
    ...
    # OAuth 2.0 authentication
        ...
        if available_scopes:
            # Initialize session state for scopes if not exists
            if "selected_scopes" not in st.session_state:
                st.session_state.selected_scopes = []
            ...
            with col1:
                if st.button("✅ Select All Scopes"):
                    st.session_state.selected_scopes = list(available_scopes.keys())
                    st.rerun()
            with col2:
                if st.button("❌ Deselect All Scopes"):
                    st.session_state.selected_scopes = []
                    st.rerun()

            selected_scopes = st.multiselect(
                f"Scopes ({len(st.session_state.selected_scopes)}/{len(available_scopes)} selected)",
                options=list(available_scopes.keys()),
                default=st.session_state.selected_scopes,
                help="Select required OAuth scopes. Use buttons above to quickly select/deselect all.",
            )
...
def main() -> None:
    ...
    if not tenants:
        st.error("⚠️ No active tenants found. Please create a tenant first in the Tenants page.")
        return

    tenant_options = {f"{name} ({tenant_id})": tenant_id for tenant_id, name in tenants}
    selected_option = st.selectbox(
        "Choose tenant for this tool",
        options=list(tenant_options.keys()),
        help="Tools are isolated per tenant for multi-tenancy support"
    )
    st.session_state.selected_tenant_id = tenant_options[selected_option]
    ...
    if st.session_state.spec_metadata:
        ...
        if st.button("💾 Save Tool", type="primary"):
            ...
            if not st.session_state.selected_tenant_id:
            ...
            with st.spinner("Saving tool and generating MCP tools..."):
                ...
                if save_result:
                    ...
                    st.balloons()
                    # Reset state
                    for key in ["uploaded_file_content", "parsed_spec", "spec_metadata", "auth_config", "selected_scopes"]:
                        if key == "selected_scopes":
