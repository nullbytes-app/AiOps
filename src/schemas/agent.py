...
class LLMConfig(BaseModel):
    ...
    def validate_model(cls, v: str) -> str:
        ...
        DYNAMIC VALIDATION: This validator only checks basic format requirements.
        The actual model availability is validated dynamically by LiteLLM proxy,
        which serves as the source of truth for supported models. This prevents
        the need to maintain a hardcoded whitelist that gets out of sync with
        available models.
        ...
        Raises:
            ValueError: If model identifier format is invalid

        Note:
            - Model availability is checked at runtime by LiteLLM
            - See /api/llm-models/available for current model list
            - Invalid models will fail at execution time with clear error messages
        """
        import logging
        ...
        # Validate reasonable length (already enforced by Field, but explicit check)
        if len(v) > 100:
            raise ValueError("Model identifier too long (max 100 characters)")

        # Validate no obvious injection attempts
        if any(char in v for char in ['\n', '\r', '\0', ';']):
            raise ValueError("Model identifier contains invalid characters")

        # Log for observability but don't block
        logger.debug(f"Model validation passed for: {v}")
...
class MCPToolAssignment(BaseModel):
    """
    Represents an MCP tool assignment for an agent.
    ...
    id: str = Field(description="Deterministic UUID5 from UnifiedToolService (server + tool name)")
    name: str = Field(description="Tool/resource/prompt name from MCP server discovery")
    source_type: Literal["mcp"] = Field(description="Source type, always 'mcp' for this schema")
    mcp_server_id: UUID = Field(description="UUID of the MCP server providing this tool")
    mcp_server_name: str = Field(description="Human-readable server name for UI display")
    mcp_primitive_type: Literal["tool", "resource", "prompt"] = Field(
        description="Type of MCP primitive"
    )
    ...
    def validate_source_type(cls, v: str) -> str:
        """Ensure source_type is always 'mcp'."""
        if v != "mcp":
            raise ValueError("source_type must be 'mcp' for MCPToolAssignment")
        return v
    ...
    def validate_uuid_format(cls, v: str) -> str:
        ...
        try:
            ...
            raise ValueError(f"id must be valid UUID string: {e}") from e
        return v
...
class AgentCreate(BaseModel):
    ...
    tool_ids: list[str] = Field(
        ...
        max_length=20,
        description="OpenAPI tool IDs (from openapi_tools table)",
    )
    mcp_tool_assignments: list[MCPToolAssignment] = Field(
        default_factory=list,
        description="MCP tool assignments (tools, resources, prompts from MCP servers)",
    )
    memory_config: Optional[MemoryConfig] = Field(default_factory=MemoryConfig)
...
class AgentUpdate(BaseModel):
    ...
    mcp_tool_assignments: Optional[list[MCPToolAssignment]] = Field(
        None, description="MCP tool assignments (tools, resources, prompts from MCP servers)"
    )
...
class AgentResponse(BaseModel):
    ...
    @classmethod
    def serialize_orm_relationships(cls, data: Any) -> Any:
        """
        Convert ORM objects and relationships to dicts for Pydantic validation.
        ...
        if hasattr(data, "__dict__"):  # It's an ORM object
                ...
                data_dict["triggers"] = [
                    {
                        ...
                        "trigger_type": (
                            ...
                            if hasattr(t.trigger_type, "value")
                            else str(t.trigger_type)
                        ),
            ...
            # Serialize tools relationship (list of AgentTool ORM objects -> list of tool_ids)
            if hasattr(data, "tools") and data.tools:
                data_dict["tool_ids"] = [t.tool_id for t in data.tools]
            else:
                data_dict["tool_ids"] = []

            # Deserialize assigned_mcp_tools JSONB field (list of dicts -> list of MCPToolAssignment)
            if hasattr(data, "assigned_mcp_tools") and data.assigned_mcp_tools:
                data_dict["mcp_tool_assignments"] = data.assigned_mcp_tools
            else:
                data_dict["mcp_tool_assignments"] = []
...
class PromptTestResponse(BaseModel):
    ...
    tokens_used: dict[str, int] = Field(
        ...,
        json_schema_extra={"example": {"input": 100, "output": 250}},
    )
